...
public class NotificationServiceImpl implements  NotificationService{
	...
	@Autowired
	public NotificationServiceImpl(NotificationRepo notificationRepo, UserService userService,AuthendicationFacade authendicationFacade) {
	...
	public List<Notification> getAllNotification(SearchCredentialDTO searchCredentialDTO) {
		...
		{
			searchCredentialDTO.setSize(10);
		}
		...
		try {
			...
			if(searchCredentialDTO.getNotificationLatUpdateDate() != null) {
				Integer count = notificationRepo.getUpdateAllNotification(
					logedInUser.getId(),
					searchCredentialDTO.getSendFrom(),
					searchCredentialDTO.getSendTo(),
					searchCredentialDTO.getFromDate(),
					searchCredentialDTO.getToDate(),
					searchCredentialDTO.isFavorite(),
					searchCredentialDTO.isReaded(),
					searchCredentialDTO.getSearchKeyWord(),
					searchCredentialDTO.getNotificationLatUpdateDate()
					);
			...
			return notificationRepo.fingAllNotifications(
			        logedInUser.getId(),
					searchCredentialDTO.getSendFrom(),
					searchCredentialDTO.getSendTo(),
					searchCredentialDTO.getFromDate(),
	...
	public Notification sendReprintNotification(Invoice invoice) {
		...
		notificationMessageMap.setMessage("Inoice Id "+invoice.getInvoiceId()+ " is reprinted by "+logedInUser.getUserId());
